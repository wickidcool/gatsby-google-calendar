...
const getSlug = event => {
  const summary = event.summary.split(" ").map(word => {
    return word.toLowerCase().split("").filter(char => !forbiddenChars.includes(char)).join("");
  }).join("-");
  const date = event.start.date ? event.start.date : moment(event.start.dateTime).format("MM-DD-YYYY");
  return `${date}/${summary}`;
};
    ...
    try {
      ...
      calendar.events.list({
        ...
        if (events.length) {
          try {
            events.map(event => {
              const nodeData = { ...processEvents(event, includedFields),
                slug: getSlug(event),
